import type { SiteNavItem } from "./blog";
...
interface LanguageSwitchHtmlOptions {
	label: string;
...
interface SocialLinksHtmlOptions {
...
export function buildBrandHtml({
	siteTitle,
	logoUrl,
	homeHref,
	escapeHtml,
}: BrandHtmlOptions): string {
	const siteTitleHtml = escapeHtml(siteTitle);
	const logoAltHtml = escapeHtml(siteTitle);
	const resolvedHomeHref = escapeHtml(homeHref);
...
export function buildLanguageSwitchHtml({
	...
	escapeHtml,
	fullWidth = false,
}: LanguageSwitchHtmlOptions): string {
	...
	return `<label class="language-switch${fullWidth ? " language-switch-full" : ""}"><span class="visually-hidden">${escapeHtml(label)}</span><select class="form-select form-select-sm language-switch-select ${fullWidth ? "w-100" : "w-auto"}" aria-label="${escapeHtml(label)}" data-language-switch>${optionsHtml}</select></label>`;
}

export function buildNavItemsHtml({
	items,
	language,
	currentPathname = "",
	resolveHref,
	escapeHtml,
}: NavItemsHtmlOptions): string {
	const getNavLabel = (item: SiteNavItem) =>
	...
	return `
		<div class="site-social-links" aria-label="Social links">
			...
			<a href="https://twitter.com/astrodotbuild" target="_blank" rel="noopener noreferrer">
				<span class="sr-only">${escapeHtml(twitterLabel)}</span>
			...
			<a href="https://github.com/withastro/astro" target="_blank" rel="noopener noreferrer">
				<span class="sr-only">${escapeHtml(githubLabel)}</span>
